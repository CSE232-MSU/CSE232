...
int main() {
  ...
  if (mark != v_i.end()){
    ...
    if (mark2 != v_i.end())
      cout <<"Found "<<target_i<<" at index "<< distance( v_i.begin(), mark2 )<<endl;
  }

  // find_if, unary predicate
  mark = find_if(v_i.begin(), v_i.end(), divisible_2_3<int>);
  if (mark != v_i.end())
    cout << "Found divis 2 and 3: " << *mark << " at index:"
          << distance( v_i.begin(), mark ) <<endl;
  ...
  string target = "this";
  auto pos = search(source.begin(), source.end(),
